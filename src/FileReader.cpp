...
//---------- R�alisation de la classe <FileReader> (fichier FileReader.cpp) --
...
bool FileReader::OpenCsvMesure(string chemin)
{
	...
	if (!csvMesure)
	{
		cerr << "Erreur lors de l'ouverture, r�essayer" << endl;
...
void FileReader::initDatesLimites()
{
	...
	//placement � la fin du fichier
	csvMesure.seekg(-30, ios::end);//offset � modifier si besoin/essayer d'arriver au milieu de l'avant derni�re ligne
	getline(csvMesure, buffer);//virer l'avant derni�re ligne
...
{
	string descriptions; //chaine � retirer lors des manipulations de fichiers
	csvMesure.seekg(0, ios::beg);
